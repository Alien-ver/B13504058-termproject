...
def display_end_options(message):
    ...
    screen.fill(WHITE)
    text_surface = win_font.render(message, True, GREEN)
    screen.blit(
        text_surface,
    ...
    while bet > 0:
        ...
        while running:
            ...
            if player_x == end_x and player_y == end_y:
                ...
                time_limit = max(20, time_limit - 5)  # 降低時間限制，但最小為20秒
                running = False

            # Check if time exceeded the current limit
        ...
        if time_limit == 20:
            display_end_options("You are too strong! Goodbye!")
            break

        if bet == 0:
            print("You have lost all your bets! Game Over.")
            break

        replay = display_end_options(f"Time: {time_elapsed:.1f}s, Steps: {steps}, Remaining Bet: ${bet}")
        if not replay:
            break
